...
public class Explosion : MonoBehaviour
{
    ...
    public void Initiation(float SoundVolume = 1f, AudioClip clip = null)
    {
        if (audioSource != null)
        {
            ...
            isPlay = true;
            audioSource.volume =
                ExplosionVolumeOffset
                * AudioDefault.MasterVolume
                * AudioDefault.EffectVolume
                * SoundVolume;
        }
    }
        ...
        if (animator.GetCurrentAnimatorStateInfo(0).IsName("Explosion"))
        {
            if (animator.GetCurrentAnimatorStateInfo(0).normalizedTime < Judgetime)
            {
                circleCollider.radius =
                    JudgeStartRadius
                        * (Judgetime - animator.GetCurrentAnimatorStateInfo(0).normalizedTime)
                        / Judgetime
                    + JudgeEndRadius
