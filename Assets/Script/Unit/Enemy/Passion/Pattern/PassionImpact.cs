...
public class PassionImpact : PatternDefault
{
    ...
    public override void Run()
    {
        ...
        {
            if (maxDistance < (pair - GameController.GetPlayer.GetComponent<PlayerPhysical>().TargettingPos).magnitude) 
            {
                maxDistance = (pair - GameController.GetPlayer.GetComponent<PlayerPhysical>().TargettingPos).magnitude;
                targetPosition = pair;
            }
        }

        gameObject.GetComponent<Rigidbody2D>().velocity = Ballistics.Ballistic(targetPosition - gameObject.transform.position,
           JumpSpeed, GameController.GetGameController.GRAVITY);
    }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.collider.CompareTag("Block"))
        {
            ...
            if (step == 0)
            {
                step = 1;
                timer = 0;
                targettingSignObject.GetComponent<SpriteRenderer>().color = TargettingSignColor - new Color(0, 0, 0, 1);
    ...
    void Update()
    {
        if (IsRun) {
            ...
            if (beam != null && beam.activeSelf) {
                ...
                beam.GetComponent<LineRenderer>().SetPosition(1, transform.position + new Vector3(0, 0, 1));
            }
            ...
            switch (step) { 
                ...
                case 0:
                    ...
                    break;
                case 1:
                    float ratio = timer / TargettingTime;
                    if (ratio < 1) {

                        targettingSignObject.GetComponent<SpriteRenderer>().color = TargettingSignColor - new Color(0, 0, 0, 0.5f - ratio);
                        targettingSignObject.transform.localScale = TargettingSignModel.transform.localScale * (1 - ratio);
                        targettingSignObject.transform.position = GameController.GetPlayer.GetComponent<PlayerPhysical>().TargettingPos;
                        if (beam != null && beam.activeSelf)
                        {
                    ...
                    else 
                    {
                        ...
                        if (GameController.GetPlayer.GetComponent<PlayerAction>().IsParrying())
                        {
                            ...
                            impactPoint = GameController.GetPlayer.GetComponent<PlayerPhysical>().TargettingPos;

                            GameController.GetPlayer.transform.position = impactPoint + new Vector3(1, 0, 0);
                        ...
                        else
                        {
                            ...
                            nomalVec = (GameController.GetPlayer.GetComponent<PlayerPhysical>().TargettingPos - gameObject.transform.position).normalized;
                            rushLeng = Mathf.Max(RushMaxLength,
                                (GameController.GetPlayer.GetComponent<PlayerPhysical>().TargettingPos - gameObject.transform.position).magnitude);
                ...
                case 3:
                    ...
                    if (nowGauge > GaugeAmont) {
                        ...
                        //fail event
                        GameController.GetPlayer.GetComponent<PlayerPhysical>().Bind(10);
                        Stop();
                    }
